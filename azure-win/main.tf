...
resource "azurerm_virtual_machine" "main" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i hosts  ../ansible/wintemp.yml"
  }
  provisioner "local-exec" {
    command = "rm -rf hosts"
  }
 ...
 /* provisioner "local-exec" {
  ...
  provisioner "local-exec" {
    command = "az disk create --resource-group ${azurerm_resource_group.main.name} --name ${var.diskimage} --source ${var.snapshot}"
  }
