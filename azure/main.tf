...
resource "azurerm_linux_virtual_machine" "main" {
  ...
  provisioner "local-exec" {
    command = "az image create --resource-group ${azurerm_resource_group.main.name} --name prathameshimage2 --source ${azurerm_linux_virtual_machine.main.name}"
    
  }
  
}
/*resource "azurerm_image" "windows-image" {
  name                      = "ubuntu-prathamesh"
  location                  = "West US"
  resource_group_name       = azurerm_resource_group.main.name
  source_virtual_machine_id = azurerm_linux_virtual_machine.main.name
}*/